    ...
    try:
        ...
        if exchange.upper() in CRYPTO_EXCHANGES:
            ...
            try:
                ...
                return (
                    False,
                    {"status": "error", "message": f"Failed to fetch LTP for {quote_symbol}: {exc}"},
                    500,
                )
        else:
            ...
            try:
                ...
                for _item in symbols_to_fetch:
                    try:
                        ...
                        logger.warning(f"[CRYPTO] Quote error for {_item['symbol']}: {quote_exc}")
                        _results.append(
                            {
                                "symbol": _item["symbol"],
                                "exchange": _item["exchange"],
                ...
                return (
                    False,
                    {"status": "error", "message": f"Failed to fetch option quotes: {exc}"},
                    500,
                )
        else:
